...
public class NotificationTaskNotifier {
    ...
    public void task(){
        notificationTaskRepository.findAllByDateTime(LocalDateTime.now().truncatedTo(ChronoUnit.MINUTES)).forEach(
                notificationTask -> {
                    telegramBotService.sendMessage(notificationTask.getChatId(), notificationTask.getText());
                }
        );
    }
